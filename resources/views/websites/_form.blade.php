...
<div class="space-y-5">
    <label class="block">
        ...
        <input name="name" value="{{ old('name', $website?->name) }}" required class="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 outline-none focus:border-zinc-500">
        @error('name') <span class="mt-1 block text-sm text-red-400">{{ $message }}</span> @enderror
    </label>

    <label class="block">
        <span class="text-sm text-zinc-300">Domain</span>
        <input name="domain" value="{{ old('domain', $website?->domain) }}" placeholder="example.com" required class="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 outline-none focus:border-zinc-500">
        @error('domain') <span class="mt-1 block text-sm text-red-400">{{ $message }}</span> @enderror
    </label>

    <label class="block">
        <span class="text-sm text-zinc-300">Timezone</span>
        <input name="timezone" value="{{ old('timezone', $website?->timezone ?? 'UTC') }}" placeholder="Europe/Berlin" required class="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 outline-none focus:border-zinc-500">
